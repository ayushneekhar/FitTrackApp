...
export const useExerciseManagement = ({
  ...
  const addExercises = useCallback((selectedExercises: Exercise[]) => {
    const newWorkoutExercises: WorkoutExercise[] = selectedExercises.map(ex => ({
      ...ex,
  ...
  const removeExercise = useCallback((instanceIdToRemove: string) => {
    setExercises(prev => prev.filter(ex => ex.instanceId !== instanceIdToRemove));
  }, []);
  ...
  const updateReps = useCallback((instanceId: string, setId: string, newReps: string) => {
  ...
  const updateWeight = useCallback((instanceId: string, setId: string, newWeight: string) => {
    const cleanedWeight = newWeight.replace(/[^0-9.]/g, "");
    const parts = cleanedWeight.split(".");
    const validatedWeight = parts.length > 1 ? `${parts[0]}.${parts.slice(1).join("")}` : cleanedWeight;
  ...
  const updateUnit = useCallback((instanceId: string, setId: string, newUnit: WeightUnit) => {
    setExercises(prevExercises =>
      prevExercises.map(ex => {
        if (ex.instanceId === instanceId) {
          const updatedSets = ex.sets.map(set => {
